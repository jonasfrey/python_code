...
class App(QWidget):
    ...
    def setup_cam_gui(self):
        ...
        for cam in self.cameras: 
            ...
            setattr(gui_object, 'width',555)
            setattr(gui_object, 'height',555)

            # setattr(self, str(cam.id), "cam_id:"+str(cam.id))

            # for (camera) in self.cameras:
            for obj in cam.camera_options:
                if( "int" in obj.datatype): 
                    ...
                    #print(obj.name)
                    path1 = "cameras[0]."+str(obj.name)
                    path3 = "cameras[1]."+str(obj.name)
    ...
    def convert_cv_qt(self, cv_img):
        ...
        bytes_per_line = ch * w
        convert_to_Qt_format = QtGui.QImage(rgb_image.data, w, h, bytes_per_line, QtGui.QImage.Format_RGB888)
        p = convert_to_Qt_format.scaled(444,444, Qt.KeepAspectRatio)
        return QPixmap.fromImage(p)
