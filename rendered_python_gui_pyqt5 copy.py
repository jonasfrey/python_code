...
class Pyqt5_view:
    ...
    def recursive_check_rendered_objects(self, original_object):
        f = open("recursive_check_rendered_objects_json.json", "a")
        f.write(json.dumps(original_object))
        f.close()
    ...
    def recursive_update_rendered_objects(self, original_object, parent_object=None):
        ...
        if(parent_object == None):
            ...
            code_statements_before_string_evaluation = parent_object['code_statements_before_string_evaluation']

        original_object['code_statements_before_string_evaluation'] = code_statements_before_string_evaluation

        if('for' in original_object):
            if('rendered_objects' not in original_object):
            ...
            # # some array items were removed , data array is now smaller than rendered objects array
            # if(evaluated_array_var_len < len(original_object['rendered_objects'])):
                
            #     n_items_lost = len(original_object['rendered_objects']) - evaluated_array_var_len
            #     for i in range(0,n_items_lost):
            #         original_object['rendered_objects'].pop(
            #             len(original_object['rendered_objects'])-1
            #         )
            # else:
            # items have been added to data array
            for key in range(0, evaluated_array_var_len):
                
                # since we are in a for loop we have to copy again
                original_object_copy = original_object.copy()
        ...
        # def self_recursive_update_rendered_objects(object, parent_object):
        #     self.recursive_update_rendered_objects(object, parent_object)

        # self.foreach_rendered_object(original_object, self_recursive_update_rendered_objects)
                ...
                if 'c' in rendered_object:
                    if(type(rendered_object['c']) == list): 
                        for child_original_object in rendered_object['c']:
                            self.recursive_update_rendered_objects(child_original_object, rendered_object)
        else:
            if 'c' in original_object:
                if(type(original_object['c']) == list): 
                    for child_original_object in original_object['c']:
                        self.recursive_update_rendered_objects(child_original_object, original_object)  
        ...
        if('for' in object):
            for rendered_object in object['rendered_objects']:
                if 'c' in rendered_object:
                    if(type(rendered_object['c']) == list): 
                        for child_original_object in rendered_object['c']:
                            callback(child_original_object, rendered_object)
    ...
    def create_pyqt5_view_object_if_not_yet_existing(self, object):
        if('pyqt5_view_object' in object):
            return 'pyqt5_view_object already existing'
        else:
